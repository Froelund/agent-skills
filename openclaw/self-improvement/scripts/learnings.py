...
def build_parser() -> argparse.ArgumentParser:
    ...
    search_parser.add_argument("--kind", choices=["learning", "error", "feature"], default=None)
    search_parser.add_argument("--limit", type=int, default=10)
    add_common_output_arg(search_parser)
    search_parser.set_defaults(func=cmd_search)

    ll_parser = subparsers.add_parser("log-learning")
    ll_parser.add_argument("--root", required=True)
    ...
    le_parser = subparsers.add_parser("log-error")
    le_parser.add_argument("--root", required=True)
    le_parser.add_argument("--name", required=True)
    ...
    lf_parser = subparsers.add_parser("log-feature")
    lf_parser.add_argument("--root", required=True)
    lf_parser.add_argument("--capability", required=True)
...
"""
    ...
    return {"ok": True, "action": "dry_run" if args.dry_run else "logged", "kind": "learning", "id": entry_id, "target_file": str(target_file), "summary": args.summary}
...
def cmd_log_error(args):
    ...
    metadata_lines = [f"- Reproducible: {args.reproducible}", f"- Related Files: {related_files}" if related_files else "", f"- See Also: {see_also}" if see_also else ""]
...
"""
    if not args.dry_run:
        append_entry(target_file, entry)
    return {"ok": True, "action": "dry_run" if args.dry_run else "logged", "kind": "error", "id": entry_id}
...
def cmd_log_feature(args):
    ...
    suggested_impl = read_text_or_file(args.suggested_implementation, args.suggested_implementation_file, "suggested-implementation")
    ...
    metadata_lines = [f"- Frequency: {args.frequency}", f"- Related Features: {related_features}" if related_features else ""]
...
### User Context
{user_context or '[TODO]'}

### Complexity Estimate
{args.complexity_estimate}

### Suggested Implementation
{suggested_impl or '[TODO]'}
...
"""
    if not args.dry_run:
        append_entry(target_file, entry)
    return {"ok": True, "action": "dry_run" if args.dry_run else "logged", "kind": "feature", "id": entry_id}
